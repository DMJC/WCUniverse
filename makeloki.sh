...
OLDVERSIONTEXT="wcuniverse" # The hidden folder in the data dir, with the m3us
NEWVERSIONTEXT="wcuniverse" # What you want the hidden folder to be called
...
# Create the packages
  ...
  sh makeself.sh --bzip2 $BASEDIR $CURPWD/../privgold-1.0.3.run.bz2.sh "Wing Commander Universe 1.00 - Base" sh setup.sh
  echo "Making the Music Installer"
  sh makeself.sh --bzip2 $MUSICDIR $CURPWD/../privgold-music.run.bz2.sh "Wing Commander Universe 1.00 - Music" sh setup.sh
  cd $CURPWD
fi
